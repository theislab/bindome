...
class REMAP:
    ...
    def get_remap_peaks(tf, genome='hg19', check=True, summit_extend=100, remove_non_canonical_chromosomes=True,
        ...
        if genome != 'hg19':
            ...
            header = 0
            if tf is not None:
                p = REMAP.get_remap_peaks_path(tf, basedir=basedir)
            else:
            ...
            df = pd.read_csv(p, header=header, sep='\t')
            ...
            df['summit.start'] = df['6'] - summit_extend
            df['summit.end'] = df['6'] + summit_extend
    ...
    def intersect_peaks_remap(tf1, tf2):
        df1 = REMAP.get_remap_peaks(tf1)
        df2 = REMAP.get_remap_peaks(tf2)

        df1 = df1[df1['chr'] != 'chrM'].reset_index(drop=True)
        df2 = df2[df2['chr'] != 'chrM'].reset_index(drop=True)

        out1 = PeaksAnalyzer.intersect_coordinates(df1[['chr', 'summit.start', 'summit.end', 'k.summit']],
                                                   df2[['chr', 'summit.start', 'summit.end', 'k.summit']])
        df3 = DataFrameAnalyzer.read_tsv(out1[-1], header=None,
                                         columns=['chr', 'summit.start', 'summit.end', 'k.summit'])
        df3 = SequenceMethods.parse_range2coordinate(df3, ['chr', 'summit.start', 'summit.end'], 'k')
        out2 = PeaksAnalyzer.intersect_coordinates(df2[['chr', 'summit.start', 'summit.end', 'k.summit']],
                                                   df1[['chr', 'summit.start', 'summit.end', 'k.summit']])
        df4 = DataFrameAnalyzer.read_tsv(out2[-1], header=None,
                                         columns=['chr', 'summit.start', 'summit.end', 'k.summit'])
    ...
    def get_great_regions(tf_name):
        path = join('/g/scb2/zaugg/rio/EclipseProjects/zaugglab/comb-TF-binding',
    ...
    def get_narrowpeaks_fasta(code, n_peaks, peak_size=200):
        ...
        if True or not exists(hg19_fa_path):
            ...
            chain_hg38_to_hg19 = '/g/scb2/zaugg/rio/EclipseProjects/zaugglab/invitro_vs_multiomics/data/hg38ToHg19.over.chain.gz'
            cmd = ['/g/software/bin/liftOver', bed_tmp, chain_hg38_to_hg19, hg19_peaks,
    ...
    def get_intersections_vs_remap(df, bkp_path=None, overwrite=False):
        ...
        intersections_df.columns = ['chr', 'start', 'end', 'chr.remap', 'start.remap', 'end.remap', 'tf.remap',
                                    '.', '..', 'summit.start', 'summit.end', 'shape', 'overlap']
        intersections_df = intersections_df[intersections_df['overlap'] != 0]
        return intersections_df

    @staticmethod
    def get_remap_ids():
        return [tf.replace(".tsv.gz", "").upper()
                for tf in listdir(join(get_parent_path(__file__), 'data/remap2/nrPeaks'))
                                  if tf.endswith('.tsv.gz')]
    ...
    def get_remap_peaks_path(tf_name, basedir=None):
        ...
        if basedir is None:
            basedir = join(get_parent_path(__file__), 'data/remap2/nrPeaks')
        ...
        p = join(basedir, tf_id + ".tsv.gz")
        if not exists(p):
            p = join(basedir, tf_id + ".bed.gz")
        if not exists(p):
            # print exists(p), p
            if not exists(p):
    ...
    def write_peaks_as_bed(tf_name, output_basename):
        peaks = REMAP.get_remap_peaks(tf_name)
        DataFrameAnalyzer.to_tsv(peaks[['chr', 'summit.start', 'summit.end', 'k.summit']], output_basename + ".bed",
                                 header=None)
    ...
    def get_all_peaks(extend=100):
        return REMAP.get_remap_peaks(None, summit_extend=extend)

    @staticmethod
    def get_random_remap_peaks(n, ngroup=100, seed=10, query_tfs=None):
        tf_names = REMAP.get_remap_ids()
        ...
        while sum([next.shape[0] for next in final]) < n:
            ...
            if query_tfs is not None and not next_tf in query_tfs:
                continue
            next_df = REMAP.get_remap_peaks(next_tf)
    ...
    def get_closest_tss_gene(coordinates, custom_coordinates=None, genome='hg19'):
        ...
        out = PeaksAnalyzer.intersect_coordinates(coordinates[['chr', 'start', 'end']],
                                                  df[['chr', 'start', 'end', 'SYMBOL', 'ENSEMBL']],
                                                  mode='closest', additional_args=['-d'])
        res = DataFrameAnalyzer.read_tsv(out[-1], header=None,
                                         columns=['chr', 'start', 'end',
                                                  'chr.gene', 'tss.start', 'tss.end', 'symbol', 'ensembl', 'distance'])
