...
def get_overlapping_peaks(df1, df2):
    ...
    only_a = a[~a['is.merged']]
    only_b = b[~b['is.merged']]

    overlap = ab[ab['has.intersect']]    
    common1 = df1[df1['coordinate'].isin(overlap['name'])]
    common2 = df2[df2['coordinate'].isin(overlap['thickEnd'])]
    c = BedTool.from_dataframe(pd.concat([common1[['chr', 'start', 'end', 'coordinate']],
                                          common1[['chr', 'start', 'end', 'coordinate']]]).sort_values(['chr', 'start'], ascending=[True, True]))
