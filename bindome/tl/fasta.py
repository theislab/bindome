...
def convert_bed_to_bed_max_position(bed_peaks_path, bed_peaks_output_path,
                                    compression=None):
    """
...
def convert_bed_to_peaks_from_summit(bed_path, bp_flanking=50,
                              stop_at=None):
    '''
...
def scrambled_fasta_order(p, tmp_path=None, random_seed=None):
    ...
    write_fasta_from_sequences(fasta,
...
def randomize_fasta(p, tmp_path=None):
    ...
    tmp_path = tempfile.mkstemp()[1] if tmp_path is None else tmp_path
    write_fasta_from_sequences(seqs,
                                    tmp_path)
    return tmp_path
...
def convert_bed_to_fasta(bed_path_or_dataframe,
                         fasta_path, genome='hg19',
                         **kwargs):
    ...
    if overwrite or not exists(fasta_path):
        if isinstance(bed_path_or_dataframe, pd.DataFrame):
            ...
            bed_path_or_dataframe[list(bed_path_or_dataframe.columns)[:3]].to_csv(bed_path_tmp, header=None, sep='\t',
                                                                                 index=None)
        ...
        if gen_path is None: # a genome was not given as input
            ...
            if genome == 'hg19' or genome == 'hg38':
                ...
                if not exists(gen_path):
                    gen_path = "C:\\Users\\ignacio\\Dropbox\\%s\\%s.fa" % (genome, genome)
                    if not exists(gen_path):
                        gen_path = '/mnt/c/Users/ignacio.ibarra/Dropbox/annotations/%s/genome/%s.fa' % (genome, genome)
                    assert exists(gen_path)
            elif genome == 'mm10':
                gen_path = join(kwargs.get('basedir', '/g/scb2/zaugg/zaugg_shared/annotations/mm10/reference'), 'mm10.fa')
            elif genome == 'mm9':
...
def convert_bed_to_fasta_hg19(bed_path_or_df, fasta_path,
                              genome_path=None,
                              force_strand=False, **kwargs):
    '''
    ...
    args = [bin, "getfasta", "-fi", selected_path,
            "-bed", bed_path, "-fo", fasta_path] + (['-s'] if force_strand else [])
    if kwargs.get('compress', False) or fasta_path.endswith('.gz'):
        args = [bin, "getfasta", "-fi", selected_path,
                "-bed", bed_path] + (['-s'] if force_strand else []) + ["| gzip >", fasta_path]
...
def get_fastas(fasta_path, uppercase=False,
    ...
    return get_fastas_from_file(fasta_path, uppercase=uppercase, stop_at=stop_at,
                                            is_gzip=is_gzip, **kwargs)

def get_fastas_from_file(fasta_path,
